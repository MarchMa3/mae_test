...
def load_sequences_from_h5(filepath):
    ...
    with h5py.File(filepath, 'r') as f:
        ...
        padded_tokens = np.zeros((total_sequences, max_seq_len), dtype=np.float32)
        ...
        return padded_tokens, padded_masks, max_seq_len, int(vocab_size)
...
def main():
    ...
    print(f"Token range: [{tokens.min():.1f}, {tokens.max():.1f}]")
