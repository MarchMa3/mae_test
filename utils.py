...
class LabDataset(data.Dataset):
    def __init__(self, tokens, missing_mask, targets=None):
        """
        Args:
            tokens: (N, seq_len, embed_dim) from 'final_tokens'
            missing_mask: (N, seq_len) from 'missing_mask'
        """
        self.tokens = torch.from_numpy(tokens).float() if isinstance(tokens, np.ndarray) else tokens
    ...
    def __getitem__(self, idx):
        sample = {
            'embedding': self.tokens[idx], # (seq_len, embed_dim)
            'missing_mask': self.missing_mask[idx] # (seq_len)
        }
